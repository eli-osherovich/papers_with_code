...
class MovieLens:
  ...
  def __init__(self, zfile: str) -> None:
    assert os.access(
      zfile, os.R_OK
    ), f"File {zfile} either does not exist or not readable."
  ...
  def items(self) -> pd.DataFrame:
    ...
    genres = pd.get_dummies(genres.map(lambda x: x.split("|")).explode()
                           ).sum(level=0)
  ...
  def full_table(self) -> pd.DataFrame:
    ...
    df["user_ratings_mean"] += (
      (df["user_ratings_mean"] - df["rating"]) / df["user_ratings_count"]
    )

    df["item_ratings_mean"] += (
      (df["item_ratings_mean"] - df["rating"]) / df["item_ratings_count"]
    )
...
def prepare_dataset(
  filepath, ratings_per_user, users_per_batch, rng=np.random.default_rng()
):
