...
class Movielens(dataset.Dataset):
  ...
  def ratings(self) -> pd.DataFrame:
    df = self._load_file(
      self._RATINGS_FILE, ['user', 'item', 'rating', 'timestamp'],
      is_re=True,
      sep=self._SEP,
    ...
    for agg_key in ['user', 'item']:
      agg_data = df.groupby(agg_key).rating.agg(
        ['mean', 'count']).rename(columns={
  ...
  def items(self) -> pd.DataFrame:
    df = self._load_file(
      self._ITEMS_FILE, ['item', 'title', 'genres'],
      is_re=True,
      sep=self._SEP,
    ...
    genres = pd.get_dummies(
      genres.map(lambda x: x.split('|')).explode()).groupby(level=0).sum()
...
class Movielens1M(Movielens):
  ...
  def users(self) -> pd.DataFrame:
    return self._load_file(
      'ml-1m/users.dat', ['user', 'gender', 'age', 'occupation_num', 'zipcode'],
      sep=self._SEP)
...
class Movielens100K(Movielens):
  ...
  def users(self) -> pd.DataFrame:
    return self._load_file(
      'ml-100k/u.user', ['user', 'item', 'gender', 'occupation', 'zipcode'],
      sep='|')
