...
from pwc.arxiv_1711_09784 import tree
...
def main(argv):
  ...
  train_ds = (
    train_ds.map(
      lambda x, y: (x / 255, y), num_parallel_calls=tf.data.AUTOTUNE
    ).cache().shuffle(FLAGS.shuffle_buffer).batch(FLAGS.batch_size
                                                 ).prefetch(tf.data.AUTOTUNE)
  )

  val_ds = (
    val_ds.map(lambda x, y:
               (x / 255, y), num_parallel_calls=tf.data.AUTOTUNE).batch(
                 FLAGS.batch_size
